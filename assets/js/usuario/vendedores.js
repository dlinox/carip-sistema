...
$(document).ready(function () {
    ...
    $('#lider').mySelect2({url: $('#baseurl').val() + 'Usuario/s2lideresElegibles', minimumInputLength: 0, placeholder: 'Seleccione al lider'});
    $('#asesorado').mySelect2({url: $('#baseurl').val() + 'Usuario/s2noAsesorados', minimumInputLength: 0, placeholder: 'Seleccione al asesor'});
    // $('#asesorado').select2();

    $('#lider').on('select2:select', function(){
        var lider_id = $('#lider').val(), asesorado_id = $('#asesorado').val();
        if(lider_id == asesorado_id) { $('#asesorado').val(null).trigger('change'); }
    });

    $('#asesorado').on('select2:select', function(){
        var lider_id = $('#lider').val(), asesorado_id = $('#asesorado').val();
        if(lider_id == asesorado_id) { $('#lider').val(null).trigger('change'); }
    });
    ...
    $('form[action="asignar_usuario"]').submit(function () {
        ...
        $this.formPost(true, {}, function (data) {
            if (data.exito)
            {
                ...
                // var asesorado_id = $('#asesorado').val();
                // // $('.select2-results__option[aria-selected=true]').css("border", "2px solid red");
                // $('#asesorado').find('[value="' + asesorado_id + '"]').remove();
                // $('#lider').find('[value="'+asesorado_id+'"]').remove();
    ...
    {
        console.log(data);
        var remover_class =  data.Lider ? '' : 'disabled';
        ...
        html =`<a type="button" href='{baseurl}usuario/eliminar_asesor/{id}'title="remover asesor" class="btn btn-warning btn-sm eliminar ` + remover_class + `"><span class="glyphicon glyphicon-trash"></span></a>`;
