...
$(document).ready(function () {
  ...
  var $table;
  function botones(id, id_alumn, $ar) {
    html = `<a href='{baseurl}certificados/personal_crear/{id}' title="Editar Certificado" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>`;
    if (id_alumn == null) {
      html += `<a href='{baseurl}certificados/personal_crear/{id}'title="Corregir Alumno" class="btn btn-info btn-sm editar"><i class="fa fa-wrench"></i></i></a>`;
    }

    html = replaceAll(html, "{baseurl}", baseurl);
    html = replaceAll(html, "{id}", id);
    //html = replaceAll(html, "{id_alumn}", id_alumn);
    $ar.append(html);
  ...
  var $dt = $("#mitabla"),
    conf = {
      ...
      onrow: function (row, data) {
        botones(data.DT_RowId, data.DT_ID_ALUM, $(row).find("td .opts"));
      },
    };
